...
const char* tbAccessToken = "be7NiyeTBgqq3eaX2mEs";
...
void printTemperature(DeviceAddress deviceAddress)
{
  ...
  char payload[100];
  snprintf(payload, sizeof(payload), "{\"temperatureL2\": %f}", tempC);
  ...
  String name = Firebase.pushFloat("Temperature Location2", tempC);
if (Firebase.failed()) {
...
{ 
  ...
  char payload[100];
  snprintf(payload, sizeof(payload), "{\"tss Location2\": %f}", tssValue);
  mqttClient.publish("v1/devices/me/telemetry", payload);
  
  delay(2000); // Adjust the delay as needed
 String name = Firebase.pushFloat("TSS Location2", tssValue);
  // Delay before taking the next reading
  float ph=random(6,9);
  Serial.print("pH Location 2: ");
  Serial.println(ph);
   if (!mqttClient.connected()) {
    connectToThingsBoard();
  }
  ...
  char payload2[100];
  snprintf(payload2, sizeof(payload2), "{\"ph Location2\": %f}", ph);
  mqttClient.publish("v1/devices/me/telemetry", payload2);
  
  delay(2000); // Adjust the delay as needed
 String name2 = Firebase.pushFloat("pH Location2", ph);
